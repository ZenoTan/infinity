...
QueuePair::QueuePair(infinity::core::Context *context) : context(context) {
  ...
  qpInitAttributes.cap.max_send_wr =
      MAX(infinity::core::Configuration::SEND_COMPLETION_QUEUE_LENGTH, 1);
  qpInitAttributes.cap.max_send_sge =
      infinity::core::Configuration::MAX_NUMBER_OF_SGE_ELEMENTS;
  qpInitAttributes.cap.max_recv_wr =
      MAX(infinity::core::Configuration::RECV_COMPLETION_QUEUE_LENGTH, 1);
  qpInitAttributes.cap.max_recv_sge =
      infinity::core::Configuration::MAX_NUMBER_OF_SGE_ELEMENTS;
  qpInitAttributes.qp_type = IBV_QPT_RC;
  qpInitAttributes.sq_sig_all = 0;

  this->ibvQueuePair =
      ibv_create_qp(context->getProtectionDomain(), &(qpInitAttributes));
  INFINITY_ASSERT(this->ibvQueuePair != NULL,
                  "[INFINITY][QUEUES][QUEUEPAIR] Cannot create queue pair.\n");
  ...
  qpAttributes.qp_access_flags =
      IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ |
      IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_ATOMIC;

  int32_t returnValue = ibv_modify_qp(
      this->ibvQueuePair, &(qpAttributes),
...
void QueuePair::activate(uint16_t remoteDeviceId,
  ...
  qpAttributes.max_dest_rd_atomic = 16;
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Cannot transition to RTR state.\n");
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Cannot transition to RTS state.\n");
}
...
uint32_t QueuePair::getQueuePairNumber() { return this->ibvQueuePair->qp_num; }

uint32_t QueuePair::getSequenceNumber() { return this->sequenceNumber; }

void QueuePair::send(infinity::memory::Buffer *buffer,
                     infinity::requests::RequestToken *requestToken) {
  send(buffer, 0, buffer->getSizeInBytes(), OperationFlags(), requestToken);
}

void QueuePair::send(infinity::memory::Buffer *buffer, uint32_t sizeInBytes,
                     infinity::requests::RequestToken *requestToken) {
  send(buffer, 0, sizeInBytes, OperationFlags(), requestToken);
}

void QueuePair::send(infinity::memory::Buffer *buffer, uint64_t localOffset,
  ...
  int returnValue =
      ibv_post_send(this->ibvQueuePair, &workRequest, &badWorkRequest);

  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Posting send request failed. %s.\n",
      strerror(errno));

  INFINITY_DEBUG(
      "[INFINITY][QUEUES][QUEUEPAIR] Send request created (id %lu).\n",
      workRequest.wr_id);
}

void QueuePair::sendWithImmediate(
    infinity::memory::Buffer *buffer, uint64_t localOffset,
...
void QueuePair::write(infinity::memory::Buffer *buffer,
                      infinity::memory::RegionToken *destination,
                      infinity::requests::RequestToken *requestToken) {
  write(buffer, 0, destination, 0, buffer->getSizeInBytes(), OperationFlags(),
        requestToken);
  INFINITY_ASSERT(
      buffer->getSizeInBytes() <= ((uint64_t)UINT32_MAX),
      "[INFINITY][QUEUES][QUEUEPAIR] Request must be smaller or equal to "
      "UINT_32_MAX bytes. This memory region is larger. Please explicitly "
      "indicate the size of the data to transfer.\n");
}
...
void QueuePair::write(infinity::memory::Buffer *buffer, uint64_t localOffset,
                      infinity::memory::RegionToken *destination,
  ...
  INFINITY_ASSERT(sizeInBytes <=
                      destination->getRemainingSizeInBytes(remoteOffset),
                  "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while "
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Posting write request failed. %s.\n",
      strerror(errno));

  INFINITY_DEBUG(
      "[INFINITY][QUEUES][QUEUEPAIR] Write request created (id %lu).\n",
      workRequest.wr_id);
}

void QueuePair::writeWithImmediate(
    infinity::memory::Buffer *buffer, uint64_t localOffset,
    infinity::memory::RegionToken *destination, uint64_t remoteOffset,
    uint32_t sizeInBytes, uint32_t immediateValue, OperationFlags send_flags,
    infinity::requests::RequestToken *requestToken) {
...
void QueuePair::multiWrite(infinity::memory::Buffer **buffers,
                           uint32_t *sizesInBytes, uint64_t *localOffsets,
                           uint32_t numberOfElements,
                           infinity::memory::RegionToken *destination,
                           uint64_t remoteOffset, OperationFlags send_flags,
                           infinity::requests::RequestToken *requestToken) {
  ...
  struct ibv_sge *sgElements =
      (ibv_sge *)calloc(numberOfElements, sizeof(ibv_sge));
  struct ibv_send_wr workRequest;
  struct ibv_send_wr *badWorkRequest;
  ...
  INFINITY_ASSERT(
      numberOfElements <=
          infinity::core::Configuration::MAX_NUMBER_OF_SGE_ELEMENTS,
      "[INFINITY][QUEUES][QUEUEPAIR] Request contains too many SGE.\n");
  ...
  INFINITY_ASSERT(totalSizeInBytes <=
                      destination->getRemainingSizeInBytes(remoteOffset),
                  "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while "
                  "writing to remote memory.\n");
  ...
  INFINITY_DEBUG(
      "[INFINITY][QUEUES][QUEUEPAIR] Multi-Write request created (id %lu).\n",
      workRequest.wr_id);
}

void QueuePair::multiWriteWithImmediate(
    infinity::memory::Buffer **buffers, uint32_t *sizesInBytes,
    uint64_t *localOffsets, uint32_t numberOfElements,
    infinity::memory::RegionToken *destination, uint64_t remoteOffset,
...
void QueuePair::read(infinity::memory::Buffer *buffer,
...
void QueuePair::read(infinity::memory::Buffer *buffer, uint64_t localOffset,
                     infinity::memory::RegionToken *source,
                     uint64_t remoteOffset, uint32_t sizeInBytes,
                     OperationFlags send_flags,
                     infinity::requests::RequestToken *requestToken) {
...
void QueuePair::batch_read(infinity::memory::Buffer *buffer,
...
void QueuePair::compareAndSwap(infinity::memory::RegionToken *destination,
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Posting cmp-and-swp request failed. %s.\n",
      strerror(errno));

  INFINITY_DEBUG(
      "[INFINITY][QUEUES][QUEUEPAIR] Cmp-and-swp request created (id %lu).\n",
      workRequest.wr_id);
}
...
void QueuePair::compareAndSwap(infinity::memory::RegionToken *destination,
                               uint64_t compare, uint64_t swap,
                               infinity::requests::RequestToken *requestToken) {
  compareAndSwap(destination, context->defaultAtomic, compare, swap,
                 OperationFlags(), requestToken);
}

void QueuePair::fetchAndAdd(infinity::memory::RegionToken *destination,
                            uint64_t add,
                            infinity::requests::RequestToken *requestToken) {
  fetchAndAdd(destination, context->defaultAtomic, add, OperationFlags(),
              requestToken);
}
...
void QueuePair::fetchAndAdd(infinity::memory::RegionToken *destination,
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][QUEUES][QUEUEPAIR] Posting fetch-add request failed. %s.\n",
      strerror(errno));

  INFINITY_DEBUG(
      "[INFINITY][QUEUES][QUEUEPAIR] Fetch-add request created (id %lu).\n",
      workRequest.wr_id);
}
...
uint32_t QueuePair::getUserDataSize() { return this->userDataSize; }

void *QueuePair::getUserData() { return this->userData; }
