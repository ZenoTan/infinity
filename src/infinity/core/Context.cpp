...
Context::Context(uint16_t device, uint16_t devicePort) {
  ...
  INFINITY_ASSERT(numberOfInstalledDevices > 0,
                  "[INFINITY][CORE][CONTEXT] No InfiniBand devices found.\n");
  INFINITY_ASSERT(device < numberOfInstalledDevices,
                  "[INFINITY][CORE][CONTEXT] Requested device %d not found. "
                  "There are %d devices available.\n",
                  device, numberOfInstalledDevices);
  INFINITY_ASSERT(ibvDeviceList != NULL,
                  "[INFINITY][CORE][CONTEXT] Device list was NULL.\n");

  // Get IB device
  this->ibvDevice = ibvDeviceList[device];
  INFINITY_ASSERT(this->ibvDevice != NULL,
                  "[INFINITY][CORE][CONTEXT] Requested device %d was NULL.\n",
                  device);

  // Open IB device and allocate protection domain
  this->ibvContext = ibv_open_device(this->ibvDevice);
  INFINITY_ASSERT(this->ibvContext != NULL,
                  "[INFINITY][CORE][CONTEXT] Could not open device %d.\n",
                  device);
  this->ibvProtectionDomain = ibv_alloc_pd(this->ibvContext);
  INFINITY_ASSERT(
      this->ibvProtectionDomain != NULL,
      "[INFINITY][CORE][CONTEXT] Could not allocate protection domain.\n");
  ...
  this->ibvSendCompletionQueue = ibv_create_cq(
      this->ibvContext, MAX(Configuration::SEND_COMPLETION_QUEUE_LENGTH, 1),
      NULL, NULL, 0);
  this->ibvReceiveCompletionQueue = ibv_create_cq(
      this->ibvContext, MAX(Configuration::RECV_COMPLETION_QUEUE_LENGTH, 1),
      NULL, NULL, 0);
  ...
  INFINITY_ASSERT(
      this->ibvSharedReceiveQueue != NULL,
...
Context::~Context() {
  ...
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][CORE][CONTEXT] Could not delete shared receive queue\n");

  // Destroy completion queues
  returnValue = ibv_destroy_cq(this->ibvSendCompletionQueue);
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][CORE][CONTEXT] Could not delete send completion queue\n");
  returnValue = ibv_destroy_cq(this->ibvReceiveCompletionQueue);
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][CORE][CONTEXT] Could not delete receive completion queue\n");

  // Destroy protection domain
  returnValue = ibv_dealloc_pd(this->ibvProtectionDomain);
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][CORE][CONTEXT] Could not delete protection domain\n");

  // Close device
  returnValue = ibv_close_device(this->ibvContext);
  INFINITY_ASSERT(returnValue == 0,
                  "[INFINITY][CORE][CONTEXT] Could not close device\n");
}

void Context::postReceiveBuffer(infinity::memory::Buffer *buffer) {

  INFINITY_ASSERT(buffer->getSizeInBytes() <=
                      std::numeric_limits<uint32_t>::max(),
                  "[INFINITY][CORE][CONTEXT] Cannot post receive buffer which "
                  "is larger than max(uint32_t).\n");
  ...
  uint32_t returnValue =
      ibv_post_srq_recv(this->ibvSharedReceiveQueue, &wr, &badwr);
  INFINITY_ASSERT(
      returnValue == 0,
      "[INFINITY][CORE][CONTEXT] Cannot post buffer to receive queue.\n");
}

bool Context::receive(receive_element_t *receiveElement) {

  return receive(&(receiveElement->buffer), &(receiveElement->bytesWritten),
                 &(receiveElement->immediateValue),
                 &(receiveElement->immediateValueValid),
                 &(receiveElement->queuePair));
}

bool Context::receive(infinity::memory::Buffer **buffer, uint32_t *bytesWritten,
                      uint32_t *immediateValue, bool *immediateValueValid,
    ...
    if (wc.opcode == IBV_WC_RECV) {
      ...
      *(bytesWritten) = wc.byte_len;
      infinity::memory::Buffer *receiveBuffer =
          reinterpret_cast<infinity::memory::Buffer *>(wc.wr_id);
...
void Context::pollSendCompletionFast(int num, struct ibv_wc *wc) {
  int comps = 0;
  while (comps < num) {
    int new_comps =
        ibv_poll_cq(this->ibvSendCompletionQueue, num - comps, &wc[comps]);
    if (new_comps != 0) {
...
ibv_context *Context::getInfiniBandContext() { return this->ibvContext; }

uint16_t Context::getLocalDeviceId() { return this->ibvLocalDeviceId; }

uint16_t Context::getDevicePort() { return this->ibvDevicePort; }

ibv_pd *Context::getProtectionDomain() { return this->ibvProtectionDomain; }
